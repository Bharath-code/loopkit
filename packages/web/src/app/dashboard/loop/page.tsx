      ...
      <header className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-2">
        ...
        {streak !== undefined && streak > 0 && (
          <div className="flex items-center gap-3 px-4 py-2 rounded-xl border border-amber-500/30 bg-amber-500/10 glow-amber cursor-help group relative">
            ...
            <div>
              <div className="text-sm font-bold text-amber-400">{streak}-Week Streak</div>
            ...
            <div className="absolute top-full right-0 mt-2 w-48 p-3 rounded-lg bg-zinc-800 border border-zinc-700 text-xs text-zinc-300 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
              You&apos;ve successfully run `loopkit loop` and shipped your synthesis for {streak} consecutive weeks.
            </div>
          </div>
        )}
      </header>
      ...
      <div className="relative pl-4 sm:pl-0">
        ...
        <div className="space-y-12">
          {!loops || loops.length === 0 ? (
            <div className="p-6 rounded-2xl border border-zinc-800 bg-zinc-900/20 text-center">
              <p className="text-zinc-500 text-sm">No loop history yet. Run <code className="text-violet-400">loopkit loop</code> on Sunday to create your first weekly synthesis.</p>
            </div>
          ) : (
            loops.map((loop) => (
              <div key={loop._id} className="relative flex flex-col sm:flex-row gap-6 sm:gap-12 group">
                ...
                <div className="sm:w-[120px] shrink-0 pt-1 flex sm:flex-col items-center sm:items-end sm:text-right gap-4 sm:gap-1 relative z-10">
                  ...
                  <div className="text-sm font-bold text-white">Week {loop.weekNumber}</div>
                ...
                <div className="flex-1 p-6 rounded-2xl border border-zinc-800 bg-zinc-900/20 hover:bg-zinc-900/40 transition-colors">
                  <div className="flex flex-wrap gap-4 mb-6 pb-6 border-b border-zinc-800">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-sm font-bold ${loop.shippingScore >= 80 ? 'text-emerald-400' : 'text-amber-400'}`}>{loop.shippingScore}/100</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-zinc-500 uppercase tracking-wider">Tasks Done</span>
                      <span className="text-sm font-bold text-white">{loop.tasksCompleted}</span>
                    </div>
                    {loop.overridden && (
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-zinc-500 uppercase tracking-wider">Override</span>
